...
//    _(\-/)_" ,  .   ,\  /\\\/.        |                           |
//    {(#b^d#)} .   ./,  |/\\\/         |       ATTENTION!!!        |
//    `-.(Y).-`  ,  |  , |\.-`          | THIS FILE IS FULLY GENE - |
//         /~/,_/~~~\,__.-`             | RATED BY ChatGPT 6 astra  |
...
TEST(BeladyCacheTest, MatchesFutureScanReference) {
  ...
  for (auto& key : history)
    key = static_cast<int>(random() % 17);
...
TEST(LfuCacheTest, RepeatedLookupDoesNotCallLoader) {
  int loader_calls = 0;
  cache::Lfu<int, int> cache(
      2, [&](const int& key) { return LoadPage(key) * ++loader_calls; });
...
TEST(LfuCacheTest, SupportsStringKeysAndValues) {
  auto loader = [](const std::string& key) {
    return "value:" + key;
  };
      ...
      if (found != model.end()) {
        ...
        if (model.size() == capacity) {
          auto victim = std::min_element(
              model.begin(), model.end(), [](const Entry& a, const Entry& b) {
...
TEST(TwoQueuesTest, RepeatedLookupUsesCachedPage) {
  ...
  cache::TwoQueues<int, int> cache(
      20, [&](const int& key) { return LoadPage(key) * ++loader_calls; });
...
TEST(TwoQueuesTest, InHitsDoNotChangeFifoEvictionOrder) {
  int loader_calls = 0;
  cache::TwoQueues<int, int> cache(
      20, [&](const int&) { return ++loader_calls; });  // IN holds two pages.
  EXPECT_EQ(cache.LookUpUpdate(1), 1);
  EXPECT_EQ(cache.LookUpUpdate(2), 2);
  EXPECT_EQ(cache.LookUpUpdate(1), 1);
...
TEST(TwoQueuesTest, OutHitReloadsAndPromotesPage) {
  int loader_calls = 0;
  cache::TwoQueues<int, int> cache(
      10, [&](const int&) { return ++loader_calls; });  // IN holds one page.
  EXPECT_EQ(cache.LookUpUpdate(1), 1);
  EXPECT_EQ(cache.LookUpUpdate(2), 2);
  EXPECT_EQ(cache.LookUpUpdate(1), 3);
...
TEST(LirsCacheTest, FiftyAccessesWithChangingWorkingSet) {
  ...
  const std::array<ExpectedAccess, 50> accesses = {{
      ...
      {5, 5, 5},    {4, 4, 5},    {1, 6, 6},    {1, 6, 6},    {6, 7, 7},
      {2, 8, 8},    {3, 3, 8},    {5, 9, 9},    {5, 9, 9},    {7, 10, 10},
      {8, 11, 11},  {7, 12, 12},  {9, 13, 13},  {8, 14, 14},  {6, 15, 15},
      {10, 16, 16}, {11, 17, 17}, {12, 18, 18}, {13, 19, 19}, {4, 20, 20},
      {5, 9, 20},   {4, 20, 20},  {5, 9, 20},   {14, 21, 21}, {15, 22, 22},
      {1, 23, 23},  {2, 24, 24},  {3, 25, 25},  {4, 20, 25},  {1, 26, 26},
      {2, 27, 27},  {3, 28, 28},  {4, 20, 28},  {1, 26, 28},  {2, 27, 28},
      {5, 29, 29},  {6, 30, 30},  {5, 31, 31},  {6, 32, 32},  {7, 33, 33},
      {8, 34, 34},  {5, 31, 34},  {6, 32, 34},  {7, 35, 35},  {8, 36, 36},
  }};
  int loader_calls = 0;
  auto loader = [&](const int&) {
    return ++loader_calls;
...
TEST(LirsCacheTest, HundredTwentyAccessesWithPromotionsAndStackPruning) {
  ...
  const std::array<ExpectedAccess, 120> accesses = {{
      ...
      {5, 29, 29},  {6, 30, 30},  {5, 31, 31},  {6, 32, 32},  {7, 33, 33},
      {8, 34, 34},  {5, 31, 34},  {6, 32, 34},  {7, 35, 35},  {8, 36, 36},
      {20, 37, 37}, {21, 38, 38}, {22, 39, 39}, {23, 40, 40}, {24, 41, 41},
      {25, 42, 42}, {26, 43, 43}, {27, 44, 44}, {28, 45, 45}, {29, 46, 46},
      {30, 47, 47}, {31, 48, 48}, {32, 49, 49}, {33, 50, 50}, {34, 51, 51},
      {35, 52, 52}, {36, 53, 53}, {37, 54, 54}, {38, 55, 55}, {39, 56, 56},
      {39, 56, 56}, {38, 57, 57}, {37, 58, 58}, {36, 59, 59}, {35, 60, 60},
      {34, 61, 61}, {33, 62, 62}, {32, 63, 63}, {31, 64, 64}, {30, 65, 65},
      {0, 66, 66},  {-1, 67, 67}, {-2, 68, 68}, {-3, 69, 69}, {0, 70, 70},
      {-1, 71, 71}, {-2, 72, 72}, {-3, 73, 73}, {0, 70, 73},  {-1, 71, 73},
      {-2, 72, 73}, {-3, 73, 73}, {0, 70, 73},  {-1, 71, 73}, {-2, 72, 73},
      {-3, 73, 73}, {0, 70, 73},  {-1, 71, 73}, {-2, 72, 73}, {-3, 73, 73},
      {1, 74, 74},  {1, 74, 74},  {2, 75, 75},  {2, 75, 75},  {3, 76, 76},
      {3, 76, 76},  {4, 77, 77},  {4, 77, 77},  {5, 78, 78},  {5, 78, 78},
      {6, 79, 79},  {6, 79, 79},  {7, 80, 80},  {7, 80, 80},  {8, 81, 81},
      {8, 81, 81},  {0, 82, 82},  {-1, 83, 83}, {0, 84, 84},  {-1, 85, 85},
  }};
  int loader_calls = 0;
  auto loader = [&](const int&) {
    return ++loader_calls;
